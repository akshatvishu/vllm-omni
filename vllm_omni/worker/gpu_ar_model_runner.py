        ...
        def _slice(value):
            if isinstance(value, torch.Tensor):
                if value.ndim > 0 and int(value.shape[0]) == int(num_tokens):
                    return value[logits_indices.to(value.device)]
                return value
    ...
    def execute_model(
        ...
        with record_function_or_nullcontext("gpu_model_runner: postprocess"):
            ...
            if not self.broadcast_pp_output:
                ...
                sample_logits_indices = logits_indices.to(hidden_states.device)
                sample_hidden_states = hidden_states[sample_logits_indices]
                compute_logits_input = self._build_compute_logits_input(
                    sample_hidden_states,
                    multimodal_outputs,
                    sample_logits_indices,
